...
// EventStore implements port.TransferEventStore using the transfer_events table.
...
func (s *EventStore) Append(ctx context.Context, evt transfer.TransferEvent) error {
	...
	_, err = s.db.Exec(ctx, `
		INSERT INTO transfer_events (
			id, transfer_id, event_type, state,
			amount, currency, occurred_at, metadata
		) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`,
		evt.EventID,
		evt.TransferID.String(),
		string(evt.Type),
		string(evt.State),
		evt.Amount.Amount,
		string(evt.Amount.Currency),
		evt.OccurredAt,
		meta,
	)
	return err
...
func (s *EventStore) FindByTransferID(ctx context.Context, id transfer.TransferID) ([]transfer.TransferEvent, error) {
	rows, err := s.db.Query(ctx, `
		...
		WHERE transfer_id = $1
		ORDER BY occurred_at`, id.String())
	if err != nil {
	...
	for rows.Next() {
		...
		events = append(events, transfer.TransferEvent{
			EventID:    eventID,
			TransferID: txID,
			Type:       transfer.EventType(eventType),
