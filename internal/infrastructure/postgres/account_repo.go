...
func (r *AccountRepo) FindByID(ctx context.Context, id ledger.AccountID) (*account.Account, error) {
	row := r.db.QueryRow(ctx, `
		SELECT id, owner_name, currency, status, created_at, updated_at
...
func (r *AccountRepo) Save(ctx context.Context, a *account.Account) error {
	_, err := r.db.Exec(ctx, `
		...
		a.ID.String(), a.OwnerName, string(a.Currency),
		string(a.Status), a.CreatedAt, a.UpdatedAt,
	)
	return err
}

// LockForUpdate acquires a SELECT FOR UPDATE row lock within the caller's transaction.
// This prevents two concurrent transfers from the same account both passing the balance check.
func (r *AccountRepo) LockForUpdate(ctx context.Context, id ledger.AccountID) (*account.Account, error) {
	row := r.db.QueryRow(ctx, `
		SELECT id, owner_name, currency, status, created_at, updated_at
		FROM accounts WHERE id = $1 FOR UPDATE`, id.String())
